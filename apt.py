...
if liabilities > income:
    ...
    if st.button("📊 Calculate Budget"):
        ...
        x1="/Users/macbook/Pictures/tinkherhackimages"
        x2=Image.open(x1)
        y1="/Users/macbook/Pictures/tinkherhackimages"
        y2=Image.open(y1)
        z1="/Users/macbook/Pictures/tinkherhackimages"
        z2=Image.open(z1)
        drive_links = {
            food_expense: st.image(y2),
            low_savings: ,st.image(x2)
            high_other_expenses: st.image(z2)
        }

        if "food" in expense_details and expense_details["food"] > 5000:
            st.write("🍕 **You are spending too much on food!**")
            
            st.image(food_expense, caption="Consider budgeting better for food!", use_container_width=True)

        if remaining_income < 3000:
            st.write("⚠️ **Your remaining savings are very low!**")
            st.image(low_savings, caption="Try to cut unnecessary expenses!", use_container_width=True)

        if "other" in expense_details and expense_details["other"] > 7000:
            st.write("🛒 **Your 'Other' expenses seem too high!**")
            st.image(high_other_expenses, caption="Consider tracking your expenses!", use_container_width=True)
